import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "تنبيهات أسعار الذهب والبيتكوين — مجانية على بريدك",
  description:
    "اشترك مجاناً في تنبيهات أسعار الذهب والبيتكوين — تنبيه يومي الساعة 8 صباحاً وتنبيه فوري عند بلوغ السعر المستهدف. بدون تطبيق، على بريدك مباشرة.",
  keywords: [
    "تنبيهات سعر الذهب",
    "تنبيه بيتكوين",
    "تنبيهات الأسواق",
    "سعر الذهب تنبيه",
    "gold price alert",
    "bitcoin alert",
  ],
  openGraph: {
    title: "التنبيهات الذكية — سعر الذهب",
    description: "تنبيهات يومية وسعرية مجانية على بريدك الإلكتروني",
    type: "website",
  },
  alternates: {
    canonical: "https://sardhahab.com/تنبيهات",
  },
};

export default function AlertsLayout({ children }: { children: React.ReactNode }) {
  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "الرئيسية", item: "https://sardhahab.com" },
      { "@type": "ListItem", position: 2, name: "التنبيهات الذكية", item: "https://sardhahab.com/تنبيهات" },
    ],
  };

  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />
      {children}
    </>
  );
}
