  ...
  try {
    ...
    if (hasResend) {
      ...
      await resend.emails.send({
        ...
        to: INBOX,
        replyTo: email,
        subject: `📩 رسالة جديدة من ${name} — سعر الذهب`,
        html: `
          <div dir="rtl" style="font-family:Helvetica,Arial,sans-serif;background:#0D0D0D;color:#F5F5F5;padding:32px;border-radius:16px;max-width:520px;margin:0 auto">
            <div style="text-align:center;margin-bottom:24px">
              ...
              <h1 style="color:#C9A84C;margin:8px 0 4px;font-size:20px">رسالة جديدة — سعر الذهب</h1>
              <p style="color:#777;font-size:12px;margin:0">sardhahab.com</p>
            </div>
            <div style="background:#1A1A1A;border:1px solid #2A2A2A;border-radius:12px;padding:20px;margin-bottom:16px">
              <table style="width:100%;font-size:14px;color:#CCC">
                <tr><td style="padding:6px 0;color:#888;width:100px">الاسم</td><td style="color:#F5F5F5;font-weight:bold">${name}</td></tr>
                <tr><td style="padding:6px 0;color:#888">البريد</td><td><a href="mailto:${email}" style="color:#C9A84C">${email}</a></td></tr>
              </table>
            </div>
            <div style="background:#1A1A1A;border:1px solid #2A2A2A;border-radius:12px;padding:20px">
              ...
              <p style="color:#F5F5F5;font-size:14px;line-height:1.7;margin:0;white-space:pre-wrap">${message.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</p>
            </div>
