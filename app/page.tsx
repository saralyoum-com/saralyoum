    ...
    <div dir="rtl" className="min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-4 pt-12 pb-8">
        ...
        <div className="bg-gradient-to-l from-gold/5 to-gold/10 border border-gold/20 rounded-2xl p-6 mb-12 flex flex-col md:flex-row items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-text-primary mb-1">🔔 اشترك في التنبيه اليومي</h2>
      ...
      <section className="max-w-7xl mx-auto px-4 pb-12">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/اخبار" className="text-gold hover:text-gold-light text-sm font-medium transition-colors">
            عرض الكل ←
          </Link>
        </div>
        ...
        {news.length > 0 ? (
          ...
          <div className="text-center py-12 text-text-secondary">
            ...
            <Link href="/اخبار" className="text-gold text-sm mt-2 inline-block">تصفح الأخبار</Link>
          </div>
        )}
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 pb-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Link href="/اسعار" className="bg-surface border border-border rounded-2xl p-6 hover:border-gold/30 transition-all group text-center">
            <div className="text-4xl mb-3">📊</div>
            <h3 className="text-text-primary font-bold mb-2 group-hover:text-gold transition-colors">جدول الأسعار</h3>
            <p className="text-text-secondary text-sm">عرض تفصيلي لجميع الأسعار</p>
          </Link>
          <Link href="/حاسبة-الذهب" className="bg-surface border border-border rounded-2xl p-6 hover:border-gold/30 transition-all group text-center">
            <div className="text-4xl mb-3">🧮</div>
            <h3 className="text-text-primary font-bold mb-2 group-hover:text-gold transition-colors">حاسبة الذهب</h3>
            <p className="text-text-secondary text-sm">احسب قيمة ذهبك وزكاتك</p>
          </Link>
          <Link href="/اخبار" className="bg-surface border border-border rounded-2xl p-6 hover:border-gold/30 transition-all group text-center">
            <div className="text-4xl mb-3">📰</div>
            <h3 className="text-text-primary font-bold mb-2 group-hover:text-gold transition-colors">الأخبار الاقتصادية</h3>
            <p className="text-text-secondary text-sm">آخر أخبار الأسواق</p>
          </Link>
          <Link href="/تنبيهات" className="bg-surface border border-border rounded-2xl p-6 hover:border-gold/30 transition-all group text-center">
            <div className="text-4xl mb-3">🔔</div>
            <h3 className="text-text-primary font-bold mb-2 group-hover:text-gold transition-colors">التنبيهات الذكية</h3>
            <p className="text-text-secondary text-sm">تنبيهات يومية وسعرية</p>
          </Link>
        </div>
      </section>
    </div>
