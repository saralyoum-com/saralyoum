...
export default function PricesPage({ initialGoldUSD }: { initialGoldUSD: number }) {
  const { lang } = useLang();
  ...
  return (
    <div dir={dir} className="max-w-7xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
      ...
      <div className="mt-6">
        ...
        {/* Bullion (سبائك) — uses server-provided price so it renders in SSR HTML for SEO */}
        {activeTab === "metals" && (metals?.gold.price || initialGoldUSD) > 0 && (
          <div className="mt-6">
            <BullionTable goldPriceUSD={metals?.gold.price ?? initialGoldUSD} rate={loc.rate} currency={loc.currency} />
          </div>
        )}
      </div>
